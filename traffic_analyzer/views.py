...
def analysis_results(request, analysis_id):
    ...
    analysis_data = {
        'peak_hours': {
            'start': peak_start,
            'end': peak_end,
            'count': time_slots.get(peak_slot, 0) if time_slots else 0
        },
    ...
    if max_density > 0:
        concerns.append({
            ...
            'description': f'Peak vehicle density of {max_density} vehicles per 5-minute window observed.'
        })
    ...
    if total_vehicles > 0:
        ...
        concerns.append({
            ...
            'description': f'High proportion of {max_vehicle_type[0]}s ({max_vehicle_type[1]:.1f}%) may impact traffic flow.'
        })
    
    analysis_data['concerns'] = concerns if concerns else [{
        'title': 'Low Traffic Volume',
        'description': 'Traffic volume is lower than expected.'
    }]
    ...
    if time_slots:
        recommendations.append({
            ...
            'description': f'Adjust signal timing during peak hours ({peak_start} - {peak_end}).'
        })
    
    if total_vehicles > 0:
        recommendations.append({
            'title': 'Lane Management',
            'description': 'Consider dedicated lanes for different vehicle types during high-density periods.'
        })
    
    analysis_data['recommendations'] = recommendations if recommendations else [{
        'title': 'Traffic Monitoring',
        'description': 'Continue monitoring traffic patterns for more detailed analysis.'
    }]
